...
def parse_args():
    ...
    parser.add_argument(
        "--epsilon_input", type=float, default=1e-5, help="the value of epsilon in ot."
    )
    parser.add_argument(
        "--logits_number", type=int, default=10, help="the number of logit."
    )
    ...
    def align_blending_model_logits_with_base_model_logits(
        ...
        for feature_1, feature_2 in zip(features_1, features_2):
            ...
            (
                ...
                base_model_per_step_logits=feature_1["per_step_logits"],
                base_model_per_step_indices=feature_1["per_step_indices"],
                vocab_align_type=args.vocab_align_type,
                blending_to_base_mapping=blending_to_base_mapping,
                epsilon_input = args.epsilon_input,
                logits_number = args.logits_number
            )
